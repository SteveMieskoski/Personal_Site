...
export class PersonalSite {
	constructor(THREE, TWEEN, cssContainerId, dataArray, dataObject, dataReverseIds) {
		...
		this.objects = _.clone(this.display.objects);
		this.targets = _.clone(this.display.targets);
		this.pagePlane = _.clone(this.display.pagePlane);
	...
	freshCopies() {
		this.objects = _.cloneDeep(this.display.objects);
		this.pagePlane = _.cloneDeep(this.display.pagePlane);
	}
	...
	init() {
		...
		this.mainMenuListener();
		this.tweenAnimate(target, 2000, false)
			.then(() => {
	...
	urlInit() {
		this.urlHandler.checkInitUrl(this.dataReverseIds, window.location.search.substring(1), (value) => {
			...
			console.log('checkInitUrl: value', value)
			/*if(value === "MainPage") {
		...
		this.urlHandler.handleBackForward((pageValue, beginningLocation) => {
			if (!this.initialLoad) {
				if (beginningLocation) {
					...
					//this.AnimateAddPageObjects(String(pageValue));
				}
			}
			console.log('handleBackForward: pageValue', pageValue, 'beginning location', beginningLocation);
	...
	mainMenuListener() {
		...
		$('div.element').click((evt) => {
			...
			this.keepId = $(evt.currentTarget).attr('id');
			this.scene.add(this.objects[$(evt.currentTarget).attr('id')]);
			this.displayPage($(evt.currentTarget).attr('id'));
			if (window.innerWidth > window.innerHeight) {
			}
		});
	}
	...
	// Tween Animate
	tweenAnimate(targets, duration, incrementStop) {
		return new Promise((resolve, reject) => {
			...
			if (incrementStop) {
				console.log('increment stop tru');
			...
			Render = () => {
				...
				if (counter > 90 && incrementStop) {
					tweening.stop();
					console.log('AddPageObject in TweenAnimate');

					this.AddPageObjects(this.keepId)
						.then(() =>{
							console.log('add page objects complete');
						})
				}
				this.renderer.render(this.scene, this.camera);
	...
	displayPage(fromResize) {
		let i, j, objectfly,
			selectFly = [],
			panelPositions = [],
			panelPositionList = _.cloneDeep(this.nonViewingArrange);
		...
		if (fromResize) {
			this.tweenAnimate(selectFly, 2000, false);
		} else {
			...
			this.ClearScene()
				.then(() => {
					...
					this.loadHtmlFile(templatePath, classes)
						.then(() =>{
							this.tweenAnimate(selectFly, 2000, true);
						})
				})
				.catch((err) => {
					console.log(err);
		...
		return new Promise((resolve, reject) => {
			if (_.size(this.selectedList) > 0) {
				if (_.last(this.selectedList) !== this.keepId) { // check if the selected item is the current item todo is this necessary??
					...
					for (let i = this.sceneP.children.length - 1; i >= 0; i--) {
