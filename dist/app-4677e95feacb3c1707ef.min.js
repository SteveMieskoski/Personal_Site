...
tweenAnimate.prototype = {
    ...
    Run: function Run(objects, targets, scene, camera, renderer, duration, tweenRate, keepId, incrementStop, cb) {
        ...
        var runCreateOrDestroy = __webpack_require__(3); // remove if bug is no longer present
    ...
    return function () {
        ...
        xmlRequest.onreadystatechange = function () {
            if (xmlRequest.readyState === 4 && xmlRequest.status === 200) {
                ...
                $(dialog).html(xmlRequest.responseText);
                var closeButtonScript = ['<button id="closeDialog" class="noprint" style="position: absolute; left: 2%; top: 2%;">Close</button>', '<button id="PrintDocument" class="noprint"  style="position: absolute; left: 8%; top: 2%;">Print Document</button>', '<script>', 'var closeDialog = document.getElementById("closeDialog");', 'closeDialog.addEventListener("click", function() {', 'var DialogElement = document.getElementById("infoPrintDialog");', 'infoPrintDialog.close();', 'DialogElement.remove();', ' });', 'PrintDocument.addEventListener("click", function() {', '$("button#CloseDialog").detach();', 'window.print();', ' });', '</script>'].join('\n');
...
//# sourceMappingURL=app-4677e95feacb3c1707ef.min.js.map