...
describe('Prior Career page should', function () {
    var rerun = 2;
    browser.url('/');

    it('be accessed by clicking MainPage Prior Career panel', function(){
        browser.click('.element:nth-Child(7)');
    ...
    it.skip('should have the right title', function () {
        ...
        assert.equal(title, 'Steve Mieskoski: Prior Career');
    }, rerun );

    it('display background template html content', function () {
        var value = browser.getText('h2*=PRIOR CAREER');
        assert.equal(value, 'PRIOR CAREER');
    }, rerun);
});